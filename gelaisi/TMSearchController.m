...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (0 == row){
        [[[UIAlertView alloc] initWithTitle:@"Previous Device" message:[NSString stringWithFormat:@"%@", self.dataArr[0][@"macName"]] delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil] show];
        ...
        if (getmsg != nil) {
            [[[UIAlertView alloc]initWithTitle:@"Device Infos:" message:getmsg delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil] show];
            return;
        }
        ...
        [NSURLConnection sendAsynchronousRequest:request queue:[[NSOperationQueue alloc] init] completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
            if (connectionError == nil) {
                ...
                if (error) {
                    ...
                    NSString *str = [[NSString alloc]initWithFormat:@"Brand:%@ \n Device Property:%@ \n Address:%@", company, country, address3];
                    //alert
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [[[UIAlertView alloc] initWithTitle:@"Device Info:" message:[NSString stringWithFormat:str, company, country] delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil] show];
                    });
                    //store the date
